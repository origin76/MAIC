#!/usr/bin/env python3
"""Plot full training trajectories for the representative mainline, MAIC, and QMIX."""
...
QMIX_JSON = ROOT / (
    "results/sc2/5m_vs_6m/"
    "qmix_sc2_5m6m_parallel_local_safe_2m_compare/"
    "2026-05-17_14-27-25_qmix_sc2_5m6m_parallel_local_safe_2m_compare_sc2_5m_vs_6m.json"
)

OUTPUT_PATH = ROOT / "paper/figures/generated/mainline_maic_qmix_comparison.png"
...
def build_stitched_curve(
    ...
    cont_x_raw, cont_y_raw = load_curve(cont_json)
    cont_x = cont_x_raw + cont_x_offset

    prefix_y = smooth(prefix_y, window=5)
    cont_y = smooth(cont_y_raw, window=5)

    anchor_x = prefix_x[-1]
    anchor_y = prefix_y[-1]
    cont_x, cont_y = prepend_anchor(cont_x, cont_y, anchor_x, anchor_y)

    x_all = np.concatenate([prefix_x, cont_x])
    y_all = np.concatenate([prefix_y, cont_y])
...
def main() -> None:
    ...
    qmix_x, qmix_y = load_curve(QMIX_JSON)
    qmix_y = smooth(qmix_y, window=5)

    fig, ax = plt.subplots(figsize=(10.6, 5.2))
    ax.plot(ours_x / 1e6, ours_y, color="#e45756", linewidth=2.6, label="Representative two-phase mainline")
    ax.plot(maic_x / 1e6, maic_y, color="#4c78a8", linewidth=2.3, label="MAIC continued to 2.0M")
    ax.plot(qmix_x / 1e6, qmix_y, color="#59a14f", linewidth=2.2, label="QMIX trained to 2.0M")

    ax.axvline(OURS_BRANCH_STEP / 1e6, color="#e45756", linestyle="--", linewidth=1.0, alpha=0.55)
    ax.text(
    ...
    ax.set_title("Representative mainline vs. MAIC vs. QMIX on SMAC 5m_vs_6m", fontsize=13, fontweight="bold")
    ax.set_xlabel("Env steps (M)")
    ax.set_ylabel("Test win rate")
    ax.set_xlim(left=0.0)
    ax.set_ylim(0.0, 1.0)
    ax.legend(frameon=False, ncol=1, loc="center left", bbox_to_anchor=(1.02, 0.5), borderaxespad=0.0)
