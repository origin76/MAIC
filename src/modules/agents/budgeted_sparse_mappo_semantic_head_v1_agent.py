...
class BudgetedSparseMAPPOSemanticHeadV1Agent(BudgetedSparseMAPPOAgent):
    def __init__(self, input_shape, args):
        super(BudgetedSparseMAPPOSemanticHeadV1Agent, self).__init__(input_shape, args)

        self.semantic_target_mode = getattr(args, "semantic_target_mode", "attack")
        ...
        aux_hidden_dim = getattr(
            args,
            "semantic_aux_hidden_dim",
            max(self.comm_value_dim * 2, 16),
        )

        if self.semantic_target_mode != "attack":
            raise ValueError("semantic_head_v1 currently only supports semantic_target_mode='attack'")
        ...
        self.semantic_action_predictor = nn.Sequential(
            nn.Linear(self.comm_value_dim, aux_hidden_dim),
            nn.ReLU(),
            nn.Linear(aux_hidden_dim, self.semantic_action_dim),
        )
    ...
    def forward(self, inputs, hidden_state, bs, test_mode=False, **kwargs):
        ...
        if kwargs.get("train_mode", False):
            semantic_alpha = alpha[:, :, :, self.semantic_action_head_idx].detach()
            semantic_values = values[:, :, self.semantic_action_head_idx, :]
            semantic_message = th.matmul(semantic_alpha, semantic_values)
            semantic_action_pred = th.sigmoid(
                self.semantic_action_predictor(semantic_message.reshape(bs * self.n_agents, -1))
            ...
            returns["semantic_action_attn"] = semantic_alpha
    ...
    def build_logging_payload(self, alpha, head_gates, gated_messages, semantic_message):
        logs = super().build_logging_payload(alpha, head_gates, gated_messages)
        logs["Scalar_semantic_head_message_norm"] = semantic_message.detach().norm(dim=-1).mean()
        return logs
