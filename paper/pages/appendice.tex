...
\xjtuappendixchapter{外文译文}

\titleformat{\section}[hang]{\xiaosan}{}{0em}{}[]
\titlespacing*{\section}{0em}{1.0\baselineskip}{0.6\baselineskip}
\titleformat{\subsection}{\sihao}{}{0em}{}[]
\titlespacing*{\subsection}{0em}{0.9\baselineskip}{0.5\baselineskip}

\input{pages/aaai2025_translation.tex}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{0.96\textwidth}{>{\raggedright\arraybackslash}p{4.8cm}>{\centering\arraybackslash}p{1.9cm}X}
        ...
        融合路径冲突边际损失权重 & 0.005 & 最终融合路径上的队友冲突边际损失权重 \\
        攻击路径冲突边际损失权重 & 0.02 & 仅攻击通信路径上的队友冲突边际损失权重 \\
        队友冲突目标边际 & 0.03 & 队友目标相对本地目标的目标边际 \\
        边际 Huber 平滑参数 & 0.02 & 边际损失中的 Huber 平滑参数 \\
        队友置信度阈值 & 0.70 & 仅在队友意图足够置信时施加冲突修正 \\
        本地置信度截断上界 & 0.90 & 不确定性暴露中的本地置信度上界 \\
...
其三，通过队友冲突边际损失把通信压力集中到更具协同价值的冲突状态；
其四，通过不确定性暴露避免在本地策略高置信状态上进行全局性强干预。

\xjtuappendixsection{通信隔离评估与 teammate-local 决策影响诊断细表}

本节对正文中的两类机制性评估进行补充展开。
第一类是执行期通信隔离评估，
用于判断攻击通信在保持其余网络不变时是否具有可观测边际收益；
第二类是 \textit{teammate-local} 决策影响诊断，
用于分析通信在“队友目标与本地目标冲突”的状态中是否真正改变了最终攻击决策。
...
\xjtuappendixsubsection{早期低介入基线的决策影响诊断细表}

为了说明本文修正主线所针对的具体瓶颈，
表 \ref{tab:appendix_signsplit_diag} 给出早期 \textit{sign-split} 基线在三 seed 上的
\textit{teammate-local} 决策影响诊断均值。
这一配置已经基本压住了沉默逃逸，
但通信残差仍几乎无法跨过本地攻击边界。

\begin{table}[htbp]
    \centering
    \caption{早期低介入基线的 \textit{teammate-local} 决策影响诊断均值}
    \label{tab:appendix_signsplit_diag}
    {\renewcommand{\arraystretch}{1.28}
    \setlength{\extrarowheight}{1pt}
    \setlength{\tabcolsep}{6pt}
    \begin{tabularx}{0.96\textwidth}{>{\centering\arraybackslash}p{3.6cm}>{\centering\arraybackslash}p{2.0cm}X}
        \toprule
        指标 & 均值 & 含义说明 \\
        \midrule
        冲突状态占比 & 0.1130 & 队友 top-1 目标与本地 top-1 目标不一致的有效攻击状态比例 \\
        队友 top-1 置信度 & 0.9253 & 队友聚合意图在冲突状态中的平均置信度 \\
        ...
        仅攻击路径跟随队友比例 & 0.0023 & 即使去掉最终融合抑制，仅攻击通信路径也几乎不跟随队友目标 \\
        最终融合跟随队友比例 & 0.0000 & 融合策略几乎从不跨过本地 top-1 边界 \\
...
\xjtuappendixsubsection{稳定通信介入方案的末段决策影响诊断统计}

表 \ref{tab:appendix_v5b_diag} 汇总了稳定通信介入方案在三 seed 末段的关键决策影响诊断统计。
...
\begin{table}[htbp]
    \centering
    \caption{稳定通信介入方案的末段决策影响诊断统计（3 seed 均值）}
    ...
    \begin{tabularx}{0.96\textwidth}{>{\centering\arraybackslash}p{3.6cm}>{\centering\arraybackslash}p{2.0cm}X}
        ...
        冲突状态占比末段均值 & 0.1326 & 训练后段仍存在非零队友冲突状态 \\
        队友 top-1 跟随率（仅攻击路径） & 0.0275 & 仅攻击通信路径能够有限地朝队友目标移动 \\
        队友 top-1 跟随率（最终融合） & 0.0141 & 最终融合后的跟随率仍明显偏低 \\
        保持本地目标比例（最终融合） & 0.9854 & 最终动作仍以本地策略为主导 \\
        加权本地置信度末段均值 & 0.8401 & 不确定性暴露更偏向本地较不确定的样本 \\
...
设 \(\hat{q}_i^t\) 为由真实通信权重聚合、并结合可执行攻击掩码得到的队友攻击意图分布，
...
\end{equation}
其中三项分别表示当前样本可攻击、队友意图置信度有效、以及真实通信质量满足要求。
则意图对齐损失可写为
\begin{equation}
L_{\text{align}}
=
...
\end{equation}
这里的停止梯度操作 \(\operatorname{sg}[\cdot]\) 用于固定队友目标分布，
...
\xjtuappendixsubsection{队友冲突边际损失与不确定性融合加权}

设本地策略与队友意图的 top-1 目标分别为
...
\end{equation}
这一设计意味着：
仅攻击通信路径可以在更广泛的冲突状态上学习“朝队友目标方向修正”，
而最终融合路径只在本地较不确定的状态中承受更强的队友冲突压力。
...
其次，
在冲突状态上，
最终融合策略跟随队友目标的比例与保持本地目标的比例分别定义为
\begin{equation}
R_{\text{follow-peer}}
=
\frac{
...
最后，
加权本地置信度用于衡量不确定性暴露是否真的偏向本地较不确定的样本，
其定义为
\begin{equation}
\bar{c}_{\text{local,w}}
=
