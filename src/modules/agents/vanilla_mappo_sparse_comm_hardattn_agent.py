...
class VanillaMAPPOSparseCommHardAttnAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(VanillaMAPPOSparseCommHardAttnAgent, self).__init__()
        self.args = args
        ...
        self.gumbel_tau = getattr(args, "hard_attn_gumbel_tau", 0.5)
        self.gumbel_hard = getattr(args, "hard_attn_train_hard", True)
        self.gate_floor = getattr(args, "hard_attn_gate_floor", 0.0)
        ...
        if not (0.0 <= self.gate_floor < 1.0):
            raise ValueError("hard_attn_gate_floor must be in [0, 1)")
    ...
    def forward(self, inputs, hidden_state, bs, test_mode=False, **kwargs):
        ...
        alpha = self._hard_attention(sparse_scores, test_mode=test_mode)

        head_messages = (alpha.unsqueeze(-1) * values.unsqueeze(1)).sum(dim=2)
        raw_head_gates = th.sigmoid(self.head_gate(agent_hidden)).unsqueeze(-1)
        # Affine floor keeps gradients flowing while guaranteeing a minimum communication path.
        head_gates = self.gate_floor + (1.0 - self.gate_floor) * raw_head_gates
    ...
    def _hard_attention(self, sparse_scores, test_mode=False):
        # Work in [bs, agents, heads, neighbors] layout so each head picks one teammate.
        logits = sparse_scores.permute(0, 1, 3, 2)
        if test_mode:
            selected = logits.argmax(dim=-1, keepdim=True)
            alpha = th.zeros_like(logits)
            alpha.scatter_(-1, selected, 1.0)
        else:
            alpha = F.gumbel_softmax(logits, tau=self.gumbel_tau, hard=self.gumbel_hard, dim=-1)
        return alpha.permute(0, 1, 3, 2)
    ...
    def build_logging_payload(self, alpha, head_gates, gated_messages):
        ...
        logs["Scalar_hard_attn_tau"] = alpha.new_tensor(self.gumbel_tau)
        logs["Scalar_hard_attn_gate_floor"] = alpha.new_tensor(self.gate_floor)
