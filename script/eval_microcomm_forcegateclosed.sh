...
# Usage:
#   zsh script/eval_microcomm_forcegateclosed.sh <alg_config> <checkpoint_dir> [seed] [test_nepisode] [load_step] [env_config]
#
# Example (g2c latest checkpoint):
#   zsh script/eval_microcomm_forcegateclosed.sh \
...
#     1 1024 1428765 sc2_5m_vs_6m_local
...
python src/main.py --config="${ALG_CONFIG}" --env-config="${ENV_CONFIG}" with \
  ...
  eval_force_comm_gate_closed=True
