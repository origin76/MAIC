...
from modules.critics.mappo_agentwise_centralized_orthogonal import MAPPOAgentWiseCentralizedOrthogonalCritic
...
class MAPPOAgentWiseCentralizedOfficialV2Learner(BudgetedSparseMAPPOLearner):
    def __init__(self, mac, scheme, logger, args):
        ...
        self.critic = MAPPOAgentWiseCentralizedOrthogonalCritic(scheme, args)
        ...
        self.use_explicit_active_masks = getattr(args, "use_explicit_active_masks", True)
        self.use_proper_time_limits = getattr(args, "use_proper_time_limits", True)
        self.use_bad_masks = getattr(args, "use_bad_masks", True)
        self.use_linear_lr_decay = getattr(args, "use_linear_lr_decay", True)
    ...
    def train(self, batch, t_env: int, episode_num: int):
        ...
        transition_mask = self._build_transition_mask(batch)

        active_masks = self._build_active_masks(batch)
        policy_mask = self._build_policy_mask(transition_mask, active_masks)
        critic_mask = self._build_critic_mask(transition_mask, active_masks)

        state_masks = self._build_state_masks(batch)
        bad_masks = self._build_bad_masks(batch)
        ...
        with th.no_grad():
            ...
            returns, advantages = self._build_gae_targets(
                rewards,
                state_masks,
                bad_masks,
                transition_mask,
        ...
        for epoch_idx in range(self.args.ppo_epochs):
            ...
            actor_log_stats.append({
                ...
                "active_agent_ratio": (policy_mask.sum() / transition_mask.expand(-1, -1, self.n_agents).sum().clamp(min=1.0)).item(),
    ...
    def _build_transition_mask(self, batch):
        transition_mask = batch["filled"][:, :-1].float()
        if "masks" in batch.scheme:
            next_masks = batch["masks"][:, 1:-1].float().min(dim=2).values
            transition_mask[:, 1:] = transition_mask[:, 1:] * next_masks
        else:
    ...
    def _build_active_masks(self, batch):
        if self.use_explicit_active_masks and "active_masks" in batch.scheme:
            return batch["active_masks"][:, :-1].float()
        avail_actions = batch["avail_actions"][:, :-1]
        return (avail_actions.sum(dim=-1, keepdim=True) > 1).float()

    def _build_policy_mask(self, transition_mask, active_masks):
        policy_mask = transition_mask.expand(-1, -1, self.n_agents)
        if self.use_policy_active_masks:
            policy_mask = policy_mask * active_masks.squeeze(-1)
        return policy_mask

    def _build_critic_mask(self, transition_mask, active_masks):
        critic_mask = transition_mask.unsqueeze(2).expand(-1, -1, self.n_agents, -1)
        if self.use_value_active_masks:
            critic_mask = critic_mask * active_masks
        return critic_mask

    def _build_state_masks(self, batch):
        if "masks" in batch.scheme:
            return batch["masks"].float()

        terminated = batch["terminated"][:, :-1].float()
        bs, max_t = terminated.shape[0], terminated.shape[1]
        masks = th.ones(bs, max_t + 1, self.n_agents, 1, device=batch.device)
        done = terminated.squeeze(-1).unsqueeze(-1).expand(-1, -1, self.n_agents).unsqueeze(-1)
        masks[:, 1:] = 1 - done
        return masks

    def _build_bad_masks(self, batch):
        if self.use_bad_masks and "bad_masks" in batch.scheme:
            return batch["bad_masks"].float()
        return th.ones_like(self._build_state_masks(batch))
    ...
    def _build_gae_targets(self, rewards, state_masks, bad_masks, transition_mask, values):
        values = values.squeeze(-1)
        rewards = rewards.squeeze(-1).unsqueeze(-1).expand(-1, -1, self.n_agents)
        state_masks = state_masks.squeeze(-1)
        bad_masks = bad_masks.squeeze(-1)
        transition_mask = transition_mask.squeeze(-1).unsqueeze(-1).expand(-1, -1, self.n_agents)

        advantages = th.zeros_like(rewards)
        gae = th.zeros_like(rewards[:, 0])

        for t in reversed(range(rewards.size(1))):
            delta = rewards[:, t] + self.args.gamma * values[:, t + 1] * state_masks[:, t + 1] - values[:, t]
            gae = delta + self.args.gamma * self.args.gae_lambda * state_masks[:, t + 1] * gae
            if self.use_proper_time_limits:
                gae = gae * bad_masks[:, t + 1]
            gae = gae * transition_mask[:, t]
            advantages[:, t] = gae
    ...
    def _update_learning_rate(self, t_env):
        decay = max(0.0, 1.0 - (float(t_env) / float(max(1, self.args.t_max))))
        actor_lr = self.initial_actor_lr * decay
        critic_lr = self.initial_critic_lr * decay
